...
class SkillAuditCLITests(unittest.TestCase):
    ...
    def test_default_paths_work_outside_project_directory(self):
        ...
        self.assertIn("Escudo Arcano", runtime_warning_skills)

    def test_strict_mode_is_cp1252_safe_and_fails_on_warnings(self):
        result = self._run_cli("--strict", cp1252=True)

        output = result.stdout.decode("cp1252")
        self.assertEqual(result.returncode, 2, result.stderr.decode("cp1252"))
