...
def capturar_modificadores_magicos(dono, data):
    """Captura no cast os multiplicadores magicos ativos do conjurador."""

    elemento = str(data.get("elemento", "")).upper()
    eh_magica = bool(
        data.get(
            "magica",
            elemento in ELEMENTOS_MAGICOS
            or data.get("tipo") in {"SUMMON", "TRANSFORM", "CHANNEL"},
        )
    )
    if not eh_magica or dono is None:
        return 1.0, 1.0

    obter_buffs = getattr(dono, "_buffs_validos", None)
    buffs = obter_buffs() if callable(obter_buffs) else ()
    dano = 1.0
    area = 1.0
    for buff in buffs:
        dano *= max(0.0, float(getattr(buff, "bonus_dano_magico", 1.0)))
        area *= max(0.0, float(getattr(buff, "bonus_area", 1.0)))
    return dano, area
...
class AreaEffect:
    ...
    def __init__(self, nome_skill, x, y, dono, *, subefeito=False):
        ...
        self.y = y
        self.dono = dono
        # Um cast de area possui uma identidade de impacto estavel. Areas
        # derivadas (por exemplo, os pilares de Julgamento) podem compartilhar
        # esta identidade para que sobreposicoes nao multipliquem o dano.
        self.fonte_impacto = self
    ...
    def atualizar(self, dt, alvos=None):
        ...
        if not self.ativado:
            ...
            if self.delay > 0:
                return resultados
            # Preserva a fracao do frame posterior ao fim do aviso. Isso torna
            # duracao e ticks independentes do tamanho do passo de simulacao.
        ...
        if self.raio_atual < self.raio:
            self.raio_atual = min(
                self.raio,
                self.raio_atual + self.raio * 3 * tempo_ativo,
            )
        ...
        if self.dano_por_segundo > 0 and alvos:
            ...
            ticks = int((self.tick_timer + 1e-9) / self.tick_interval)
            if ticks:
                self.tick_timer = max(
                    0.0,
        ...
        if self.stacks_por_segundo > 0 and alvos:
            for alvo in alvos:
                ...
                if dist >= self.raio_atual:
                    continue
                progresso = self.stack_progresso.get(alvo, 0.0)
                progresso += self.stacks_por_segundo * tempo_ativo
                stacks = int(progresso + 1e-9)
                self.stack_progresso[alvo] = max(0.0, progresso - stacks)
...
class Beam:
    ...
    def __init__(self, nome_skill, x_origem, y_origem, x_destino, y_destino, dono):
        self.nome = nome_skill
        data = get_skill_data(nome_skill)
        mod_dano_magico, _ = capturar_modificadores_magicos(dono, data)
        ...
        self.dano = data.get("dano", 15.0) * mod_dano_magico
...
class Buff:
    ...
    def __init__(self, nome_skill, alvo, *, rng=None):
        ...
        roubavel_solicitado = bool(data.get("roubavel", True))
        ...
        self.bonus_dano_magico = max(
            0.0,
            float(data.get("bonus_dano_magico", 1.0)),
        )
        self.bonus_area = max(0.0, float(data.get("bonus_area", 1.0)))
        self.esquivas_restantes = max(
            0,
            int(data.get("esquiva_garantida", 0)),
        )
        ...
        self.persistente = any(
            (
                self.escudo > 0.0,
                self.buff_dano != 1.0,
                self.buff_velocidade != 1.0,
                self.buff_velocidade_ataque != 1.0,
                self.refletir > 0.0,
                self.cura_por_segundo > 0.0,
                self.mod_dano_recebido != 1.0,
                self.mod_cura_recebida != 1.0,
                self.mod_cooldown != 1.0,
                self.mod_mana_custo != 1.0,
                self.lifesteal > 0.0,
                self.imortal_disponivel,
                self.voo,
                self.imune_ground,
                self.reflete_projeteis,
                self.reflete_skills,
                self.bonus_dano_magico != 1.0,
                self.bonus_area != 1.0,
                self.esquivas_restantes > 0,
                self.ve_ataques,
                self.dano_contato > 0.0,
                bool(self.modificadores_stats),
            )
        )
        self.roubavel = roubavel_solicitado and self.persistente
    ...
    def consumir_esquiva_prevista(self):
        if not self.ativo or self.esquivas_restantes <= 0:
            return False
        self.esquivas_restantes -= 1
        return True
...
class DotEffect:
    ...
    def __init__(
        ...
        *,
        ...
        self.vida = duracao
        self.cor = cor
        self.contexto_dano = contexto_dano
            ...
            if not self.alvo.morto:
                ...
                if callable(aplicar_direto):
                    aplicar_direto(
                        ...
                        flash_cor=self.cor,
                        contexto_dano=self.contexto_dano,
                    )
                else:
                    ...
                    if self.alvo.vida <= 0:
                        if self.contexto_dano is None:
                            self.alvo.morrer()
                        else:
                            self.alvo.morrer(contexto_dano=self.contexto_dano)
...
class Trap:
    ...
    def __init__(self, nome_skill, x, y, dono):
        ...
        self.largura = data.get("largura", 2.0) * mod_area_magica
        self.altura = data.get("altura", 3.0) * mod_area_magica
    ...
    def colidir_segmento(self, x1, y1, x2, y2, raio=0.0):
        """Detecta projeteis rapidos contra o retangulo sem tunneling."""

        raio = max(0.0, float(raio))
        left = self.x - self.largura / 2 - raio
        right = self.x + self.largura / 2 + raio
        top = self.y - self.altura / 2 - raio
        bottom = self.y + self.altura / 2 + raio
        if (
            left <= x1 <= right
            and top <= y1 <= bottom
            or left <= x2 <= right
...
class Channel:
    ...
    def __init__(self, nome_skill, dono):
        ...
        # Efeitos
        self.dano_por_segundo = (
            data.get("dano_por_segundo", 0) * mod_dano_magico
        )
