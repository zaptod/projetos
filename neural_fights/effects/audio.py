...
class AudioManager:
    ...
    def __init__(self):
        ...
        # Assets do pacote sao somente leitura; overrides vivem em runtime.
        self.sound_dir = str(get_runtime_sound_dir())
        self.package_sound_dir = str(PACKAGE_SOUND_DIR)
        logger.debug(
            "Sound directories: runtime=%s package=%s",
            self.sound_dir,
            self.package_sound_dir,
        )
    ...
    def _load_sound_config(self) -> dict:
        ...
        try:
            ...
            logger.warning("Configuracao de audio invalida: %s", exc)
        return {}
    
    def save_volume_config(self):
        """Salva configuração de volumes no arquivo."""
        config = self._load_sound_config()
        config["_volumes"] = {
