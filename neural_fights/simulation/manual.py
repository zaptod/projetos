...
class SimuladorManual(Simulador):
    ...
    def __init__(self, match_config=None):
        ...
        try:
            ...
            try:
                ...
                exc.add_note(f"Falha adicional ao limpar modo manual: {cleanup_error}")
            raise

    def _inicializar_modo_manual(self):
        """Inicializa controles e menus depois que a base adquiriu ownership."""
    ...
    def executar(self):
        """Executa o loop garantindo teardown em retorno ou excecao."""
    ...
    def _executar_loop_manual(self):
        ...
        while running:
            ...
            # Update normal; uma falha nao pode deixar a IA desconectada.
            try:
                self.update(dt)
            finally:
                if controlando_tinha_brain:
