...
MIDIA = {CHARACTER: IMAGEM, WEAPON: IMAGEM, CHARACTER_WEAPON: VIDEO}

PICASSO, DIGEN = "picasso", "digen"
PROVEDOR = {CHARACTER: PICASSO, WEAPON: PICASSO, CHARACTER_WEAPON: DIGEN}
...
# veio resolver.
DEPENDE_DE = {CHARACTER_WEAPON: (CHARACTER, WEAPON)}
...
def valido(slot: str) -> str:
    if slot not in SLOTS:
        raise ValueError(f"slot desconhecido: {slot!r} (esperado: {', '.join(SLOTS)})")
    return slot
...
def rotulo(slot: str) -> str:
    return {CHARACTER: "personagem", WEAPON: "arma",
            CHARACTER_WEAPON: "personagem + arma"}[valido(slot)]
